...
def load_dataset(csv_path, customer_count, random_seed):
    ...
    partial_df['Row_Date'] = pd.to_datetime(partial_df['Row_Date'])
    partial_df['Last_Date_primary'] = pd.to_datetime(partial_df['Last_Date_primary'])
    partial_df['Registration_Date'] = pd.to_datetime(partial_df['Registration_Date'])

    partial_df['Age'] = pd.to_numeric(partial_df['Age'], errors='coerce')
    partial_df['Customer_Seniority_Months'] = pd.to_numeric(partial_df['Customer_Seniority_Months'],
                                                            errors='coerce')

    partial_df['Is_Residence_Different_Bank'] = partial_df['Is_Residence_Different_Bank'].map(
        {'S': 1, 'N': 0, 1: 1, 0: 0}).astype('float64')
    partial_df['Is_Birth_Different_Bank'] = partial_df['Is_Birth_Different_Bank'].map(
        {'S': 1, 'N': 0, 1: 1, 0: 0}).astype('float64')
    partial_df['Is_Spouse_Of_Employee'] = partial_df['Is_Spouse_Of_Employee'].map(
        {'S': 1, 'N': 0, 1: 1, 0: 0}).astype('float64')
    partial_df['Is_Deceased'] = partial_df['Is_Deceased'].map({'S': 1, 'N': 0, 1: 1, 0: 0}).astype('float64')

    categorical_fields = partial_df.dtypes[partial_df.dtypes == 'object'].index

    for field in categorical_fields:
        partial_df[field] = partial_df[field].astype('category')
